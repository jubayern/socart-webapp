...
import { ChevronLeft, MapPin, CreditCard, Check, Calendar, Gift, FileText } from 'lucide-react'
...
export default function CheckoutClient() {
  ...
  useEffect(() => {
    ...
    if (c) {
      setTimeout(() => validateCoupon(c), 500)
    }
  ...
  const validateCoupon = async (code: string) => {
    ...
    try {
      const r = await api.post('/api/cart/validate-coupon', { code: code.toUpperCase(), subtotal })
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', paddingBottom: 120 }}>
      ...
      <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {addresses.map((a: any) => (
                <div
                  ...
                  style={{
                    padding: '10px 12px',
                    borderRadius: 11,
                    border: `1.5px solid ${form.name === a.name && form.phone === a.phone ? '#7c3aed' : '#e2e8f0'}`,
                    background: form.name === a.name && form.phone === a.phone ? '#faf7ff' : 'white',
                    cursor: 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 10,
        ...
        <div style={{ background: 'white', borderRadius: 18, padding: 14, border: '1px solid #f1f5f9' }}>
          ...
          >
            <MapPin size={14} color="#7c3aed" />Delivery Information
          </p>
        ...
        >
          ...
          <div>
            <p style={{ fontSize: 12, color: '#64748b', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 5 }}>
              <Calendar size={12} />Preferred Delivery Date (optional)
            </p>
            <input
            ...
            <div style={{ position: 'relative', width: 44, height: 24 }} onClick={() => set('gift_wrap', !form.gift_wrap)}>
              <div
                style={{
                  ...
                  inset: 0,
                  borderRadius: 99,
                  background: form.gift_wrap ? '#7c3aed' : '#e2e8f0',
          ...
          <div>
            <p style={{ fontSize: 12, color: '#64748b', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 5 }}>
              <FileText size={12} />Order Note (optional)
            </p>
            <textarea
              value={form.note}
        ...
        {refBalance > 0 && (
          <div style={{ background: 'white', borderRadius: 16, padding: 14, border: '1px solid #f1f5f9' }}>
              ...
              <div style={{ position: 'relative', width: 44, height: 24 }} onClick={() => set('use_ref_balance', !form.use_ref_balance)}>
                <div
                  style={{
                    ...
                    inset: 0,
                    borderRadius: 99,
                    background: form.use_ref_balance ? '#10b981' : '#e2e8f0',
                    transition: 'background .2s',
                  }}
                >
                  <div
                    style={{
                      position: 'absolute',
                      top: 2,
                      left: form.use_ref_balance ? 22 : 2,
                      width: 20,
                      height: 20,
                      borderRadius: '50%',
                      background: 'white',
            ...
            {form.use_ref_balance && refDisc > 0 && (
              <p style={{ fontSize: 12, color: '#10b981', fontWeight: 700, marginTop: 6 }}>−৳{refDisc.toFixed(2)} will be applied</p>
            )}
          </div>
        )}
        ...
        <div style={{ background: 'white', borderRadius: 18, padding: 14, border: '1px solid #f1f5f9' }}>
          ...
          >
            <CreditCard size={14} color="#7c3aed" />Payment Method
          </p>
            ...
            {PAYMENT_METHODS.map((pm) => (
              <div
                ...
                style={{
                  padding: '11px 13px',
                  borderRadius: 11,
                  border: `1.5px solid ${form.payment === pm.id ? '#7c3aed' : '#e2e8f0'}`,
                  background: form.payment === pm.id ? '#faf7ff' : 'white',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 10,
          ...
          {form.payment !== 'cod' && (
            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <input
                ...
                placeholder={`Your ${form.payment} number *`}
                style={iS}
                type="tel"
              />
        ...
        <div style={{ background: 'white', borderRadius: 18, padding: 16, border: '1px solid #f1f5f9' }}>
          ...
          {[
            ...
            <div key={l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 13 }}>
              ...
              <span style={{ fontWeight: 600, color: String(v).startsWith('-') ? '#10b981' : '#0f172a' }}>{v}</span>
            </div>
          ))}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'white', borderTop: '1px solid #f1f5f9', padding: '12px 14px' }}>
        <button
          ...
          style={{
            ...
            borderRadius: 14,
            border: 'none',
            background: placing || cart.length === 0 ? '#e2e8f0' : 'linear-gradient(135deg,#7c3aed,#4f46e5)',
            color: placing || cart.length === 0 ? '#94a3b8' : 'white',
            fontWeight: 800,
            fontSize: 15,
            cursor: 'pointer',
            boxShadow: !placing && cart.length > 0 ? '0 4px 18px rgba(109,40,217,.3)' : undefined,
          }}
        >
