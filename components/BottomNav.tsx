...
export default function BottomNav({ cartCount = 0, wishlistCount = 0 }: { cartCount?: number; wishlistCount?: number }) {
        ...
        {tabs.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href}
              ...
              {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-blue-600 rounded-b-full" />}
            </Link>
          )
        })}
      </div>
